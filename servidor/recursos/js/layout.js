...
async function loadPage(route) {
  try {
    ...
    var res = await fetch(BASE_URL + '/' + cleanRoute + '/index.html');
    if (!res.ok) {
    ...
    if (typeof cargarClientes === 'function' && cleanRoute === 'clientes') cargarClientes();
    if (typeof cargarEmpleados === 'function' && cleanRoute === 'empleados') cargarEmpleados();
...
function initAjustes() {
  ...
  if (formPass) {
    formPass.addEventListener('submit', function(e) {
      ...
      .then(function(data) {
        if (data.ok) {
          mostrarToastAjustes(data.mensaje, 'success');
          formPass.reset();
          document.querySelectorAll('.req-ok').forEach(function(el) { el.classList.remove('req-ok'); });
        } else mostrarToastAjustes(data.mensaje, 'error');
      })
      .catch(function() { mostrarToastAjustes('Error de conexion', 'error'); });
    });
  }
  ...
  if (formUser) {
    formUser.addEventListener('submit', function(e) {
      ...
      .then(function(r) { return r.json(); })
      .then(function(data) {
        if (data.ok) { usuarioOriginal = val; mostrarToastAjustes(data.mensaje, 'success'); }
        else mostrarToastAjustes(data.mensaje, 'error');
      })
      .catch(function() { mostrarToastAjustes('Error de conexion', 'error'); });
    });
...
function mostrarToastAjustes(mensaje, tipo) {
