...
function crearMiniJuego() {
  ...
  var area = overlay.querySelector('#mj-area');
  var grid = overlay.querySelector('#mj-grid');
  var paresEl = overlay.querySelector('#mj-pares');
  var intentosEl = overlay.querySelector('#mj-intentos');
  var restartBtn = overlay.querySelector('#mj-restart');
  var cerrarBtn = overlay.querySelector('#mj-cerrar');
  ...
  function startGame() {
    matchedCount = 0; moves = 0;
    flipped = []; locked = false;
    paresEl.textContent = '0';
    intentosEl.textContent = '0';
    restartBtn.style.display = 'none';
    var old = area.querySelector('.memory-complete');
    if (old) old.remove();

    var deck = [];
    for (var i = 0; i < emojis.length; i++) {
      deck.push({ id: i, emoji: emojis[i] });
      deck.push({ id: i, emoji: emojis[i] });
    }
    shuffle(deck);
    cards = deck;
    ...
    for (var k = 0; k < deck.length; k++) {
      ...
      el.onclick = function() {
        ...
        if (locked) return;
        var c = grid.children[idx];
        if (!c || c.classList.contains('is-flipped') || c.classList.contains('is-matched')) return;
        if (flipped.length >= 2) return;
        c.classList.add('is-flipped');
        flipped.push(idx);
        if (flipped.length === 2) {
          moves++;
          intentosEl.textContent = moves;
          locked = true;
          var idx0 = flipped[0], idx1 = flipped[1];
          if (cards[idx0].id === cards[idx1].id) {
            setTimeout(function() {
              grid.children[idx0].classList.add('is-matched');
              grid.children[idx1].classList.add('is-matched');
              matchedCount++;
              paresEl.textContent = matchedCount;
              flipped = [];
              locked = false;
              if (matchedCount === 6) {
                var d = document.createElement('div');
                d.className = 'memory-complete';
                d.innerHTML = '<h3>\uD83C\uDF89 Completado</h3><p>' + moves + ' intentos</p>';
                area.appendChild(d);
            ...
            setTimeout(function() {
              grid.children[idx0].classList.remove('is-flipped');
              grid.children[idx1].classList.remove('is-flipped');
              flipped = [];
              locked = false;
            }, 800);
          }
  ...
  cerrarBtn.onclick = function() { overlay.classList.remove('is-open'); };
  restartBtn.onclick = startGame;

  return { overlay: overlay, iniciar: startGame };
}
