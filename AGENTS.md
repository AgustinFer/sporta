...
- **Script loading**: each PHP page declares its own `<script>` tags. Only scripts from the **initial** page load execute. `loadPage()` never re-executes fetched scripts. To make a JS function available after SPA navigation, its `<script>` must be loaded globally from every page. For example, `turnos.js` and `canchas.js` are loaded in **every module's `index.php`** so `initTurnosPage()`/`initCanchasPage()` are globally available no matter which page the user F5'd on; `loadPage()` calls them when navigating to those routes. If you add a new module JS file, include it in all pages.
