...
- **Botón Pago en reservas** (`reservas/reservas.js:37-55, 173-176`, `reservas/reservas.css:157`): `renderTablaReservas()` calcula `total = parseFloat(factura_total || cancha_precio || 0)` y `saldo = total - total_pagado`; `pagoDeshabilitado = reser_estado === 3 || (total > 0 && saldo <= 0.01)`. Si true, renderiza `<button class="btn-pago" disabled title="Sin saldo pendiente">` (gris vía `reservas.css:157` `.btn-pago:disabled { opacity:0.4; cursor:not-allowed }`). La delegación en `click` hace `if (pagoBtn.disabled) return` para no abrir `#panelPago`. No depende de `factura_estado`, usa saldo numérico con tolerancia `0.01` (mismo umbral que `pagos` pero invertido).
...
- **Impresión en Pagos** (`pagos/pagos.js`, `pagos/pagos.css`, `componentes/drawers/pagos.html`): El drawer de factura (`#panelPago`) es el único contenido imprimible. `api/pagos.php:listar` devuelve `COALESCE(pg.total_pagado,0) AS total_pagado` vía `LEFT JOIN (SELECT factura_id, SUM(pago_monto) ...) pg` para que `renderTablaPagos()` calcule `saldo = total - pagado` sin fetch extra. Botón `Imprimir Factura` existe en tabla (junto a `Ver factura`, `.btn-imprimir`) y en drawer (`#btnImprimirFactura` + `#imprimirHint`); ambos quedan `disabled` + `title` + `mostrarToast("La factura tiene saldo pendiente — no se puede imprimir","error")` si `saldo > 0.01`. `imprimirFactura(facturaId)` usa `window.print()` directo del drawer (nunca `window.open`/`buildFacturaPrintHtml`): si drawer ya está abierto y cacheado para ese `facturaId` imprime directo, sino hace `fetch factura_detalle`, valida saldo y hace `abrirDetalleFactura(id)` + `setTimeout(print,400)`. Leyenda `<p class="print-leyenda">Documento no válido como factura.</p>` dentro de `#panelPago`, `display:none` en pantalla y visible solo en `@media print`.
- **`pagos.css` @media print**: No usar `body *{visibility:hidden}` — deja `#drawer-container` oculto y el drawer queda a 420px recortado a la derecha. Usar `display:none` para `#sidebar-container, .sidebar, .menu-toggle, .app-content, #header-container, .background, #toast-container, .drawer-overlay, #btnImprimirFactura, #imprimirHint, .drawer-close` y centrar drawer `max-width:780px; margin:0 auto; position:static; border:1px solid #e5e7eb; @page {margin:12mm; size:A4 portrait}` con `print-color-adjust:exact`.
- **Turnos soft-assist hoy / hora en curso (2026-08-29):**
  - BE `api/turnos_canchas.php:149` permite `hora_en_curso` (`inicio ≤ now < fin`) y bloquea solo `fin ≤ now`; retorna `reserva_id/factura_total/requiere_sena/requiere_pago_total`.
  - FE `recursos/js/turnos.js:602` `turnosPostReservaExito()` si `fecha===hoy` cierra drawer turnos, muestra `warning 5s` (`⚠️ Reserva creada — hoy: se requiere seña` / `hora en curso: pago total`) y abre `componentes/drawers/reservas.html#panelPago` (soft-assist, al cerrar sin pagar deja `reser_estado=1` + toast pendiente `warning 5s`).
  - `turnosCargarDatos()` es `return fetch` (awaitable); fallback si BE no trae `reserva_id` solo busca `reser_estado !==3` (nunca `turnosReservas[last]`). Ver `turnos.js:514/560`.
  - `reservas.js` real está en `reservas/reservas.js` (no `recursos/js/`), `turnos/index.html:62` y `turnosAsegurarReservasHelpers` usan `BASE_URL+'/reservas/reservas.js'`; el `<select metodo_pago>` se puebla vía `cargarMetodosPago()` primero y fallback `reservas.php?accion=listar`.
  - `global.css:568` `.toast-warning` (`#facc15`) para pendientes; `mostrarToast(msg,tipo,duracionMs)` default `3000`.
